...
from ai_client import smart_ai_call, ViralContent, SearchQueries
...
def build_description_prompt(title: str) -> str:
    """برومبت احترافي لتوليد وصف فيروسي + hashtags مع تعليمات صارمة لـ JSON."""
    return f"""أنت كاتب محتوى فيروسي متخصص في كتابة أوصاف فيديوهات قصيرة لمنصات TikTok وYouTube Shorts وFacebook Reels.

مهمتك كتابة وصف فيديو واحد + 15 هاشتاج بناءً على العنوان الذي سأعطيك إياه.

🚨 قواعد JSON الإلزامية (مهم جدًا جدًا):
- يجب أن يكون الناتج JSON صالحاً بنسبة 100%.
- ممنوع منعًا باتًا استخدام علامات الاقتباس المزدوجة (") داخل قيم النصوص العربية.
- إذا احتجت لاقتباس داخل النص، استخدم العلامة الفرنسية « » أو الفردية ' '.
- ممنوع الأسطر الجديدة داخل القيم النصية.
- اكتب الوصف كفقرة واحدة متصلة.
- تأكد من إغلاق جميع السلاسل النصية بشكل صحيح.
- لا تضع أي شيء قبل أو بعد الـ JSON.

🚨 قواعد إلزامية صارمة للوصف (description):

1. الطول الإلزامي:
- الوصف يجب أن يكون بين 100 و 140 كلمة بالضبط.
- إذا كان أقل من 100 كلمة، أضف تفاصيل نفسية وأمثلة أكثر.
- إذا كان أكثر من 140 كلمة، اختصر.
- عُدّ الكلمات قبل الإرسال.

2. اللغة:
- استخدم اللغة العربية الفصحى البسيطة فقط.
- ممنوع استخدام أي لغة أخرى.
-  اخرى يمكن استخدام  الرموز التعبيرية أو الإيموجي فقط بدون أي رموز غير نصية.

3. البنية (مهم جدًا):
- الجملة الأولى: صدمة نفسية قوية (سؤال غامض أو حقيقة صادمة).
- الجملة الثانية: تعميق الفضول مع لمحة عن السر.
- الجملة الثالثة: وعد بكشف الحقيقة الخفية.
- ثم 4-6 جمل تشرح الفكرة تدريجيًا بأسلوب غامض.
- الجملة الأخيرة: سؤال يجعل القارئ يفكر.

4. الأسلوب:
- نفسي + تحليلي + غامض + بسيط.
